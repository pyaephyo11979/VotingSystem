...
interface OutletCtx {
  ...
  setEventData: (v: EventData | null) => void;
}
...
const AdminDashboard = () => {
  ...
  const handleCandidateAdded = (newCandidate: {
    ...
    setCandidates((prev) => [
      ...prev,
      {
        id: newCandidate.candidateId,
        name: newCandidate.name,
        photo: newCandidate.hasPhoto ? "" : undefined,
      },
    ]);
  };
      ...
      <div className="w-full max-w-6xl flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          {eventId && (
            <Link
              to={`/results/${eventId}`}
              className="px-3 py-1 h-fit rounded bg-blue-600 text-white text-sm hover:bg-blue-700"
            >
              {t("live")}
            </Link>
          )}
          <button
            onClick={handleLogout}
            className="px-3 py-1 h-fit rounded bg-red-600 text-white text-sm hover:bg-red-700"
          >
            {t("logout")}
          </button>
        </div>
