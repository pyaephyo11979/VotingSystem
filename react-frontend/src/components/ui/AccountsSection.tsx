...
const AccountsSection = ({ eventId }: AccountsSectionProps) => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      show(
        `Created ${"accountsCreated" in data ? data.accountsCreated : size} account(s)`,
        { type: "success" },
      );
    } catch (err) {
  ...
  const rows = useMemo(() => {
    ...
    return accountsData.accounts.map((a) => ({
      username: a.username,
      userId: a.userId,
      password: a.password,
      eventId: a.eventId,
  ...
  const copyText = async (text: string, successMsg: string) => {
    try {
      if (typeof navigator !== "undefined" && navigator.clipboard?.writeText) {
        ...
        if (ok) show(successMsg, { type: "success" });
  ...
  const copyAll = () => {
    const text = rows
      .map(
        (r) =>
          `UserID: ${r.userId}\tUsername: ${r.username}\tPassword: ${r.password ?? ""}\tEventID: ${r.eventId}`,
      )
      .join("\n");
  ...
  return (
    <div className="p-4 border rounded-lg space-y-4 bg-white shadow-sm">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-lg font-semibold">{t("voter_accounts")}</h2>
        <div className="flex items-center gap-2 text-xs text-gray-500">
          {accountsData && <span>Total: {accountsData.accounts.length}</span>}
          {accountsData?.accountsCreated && (
            <span className="text-green-600">
              +{accountsData.accountsCreated} new
            </span>
          )}
        </div>
      </div>
      <form onSubmit={handleCreate} className="flex flex-wrap items-end gap-3">
        <label className="flex flex-col text-sm">
          <span className="mb-1 font-medium">{t("generate")}</span>
          <input
            type="number"
            min={1}
            value={size}
      ...
      <div className="text-sm max-h-64 overflow-auto border rounded p-3 bg-gray-50 relative">
        {!accountsData && !loading && (
          <p className="text-gray-500">{t("no_accounts")}</p>
        )}
        {loading && (
  ...
  const download = (format: "csv" | "json" | "txt") => {
    ...
    if (format === "json") {
      ...
      const csvRows = rows.map((r) =>
        [r.username, r.userId, r.password, r.eventId]
          .map((v) => `"${(v ?? "").replace(/"/g, '""')}"`)
          .join(","),
      );
  ...
  return (
    <div className="flex flex-wrap gap-2 mb-2">
      ...
      <button
        type="button"
        onClick={copy}
        className="px-2 py-0.5 bg-gray-300 text-gray-800 rounded text-xs hover:bg-gray-400"
      >
        Copy
      </button>
    </div>
