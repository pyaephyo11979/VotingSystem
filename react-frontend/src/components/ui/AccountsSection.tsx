  ...
  return (
    <div className="p-4 border rounded-lg space-y-4 bg-white shadow-sm">
      ...
      <form onSubmit={handleCreate} className="flex flex-wrap items-end gap-3">
        ...
        <button disabled={loading} className="bg-gray-900 text-white px-4 py-2 rounded text-sm hover:bg-gray-700 disabled:opacity-50">{loading?"Processing...":t('create')}</button>
        <button type="button" onClick={handleRefresh} disabled={loading} className="bg-gray-200 px-4 py-2 rounded text-sm hover:bg-gray-300 disabled:opacity-50">{t('refresh')}</button>
      ...
      <div className="text-sm max-h-64 overflow-auto border rounded p-3 bg-gray-50 relative">
        {!accountsData && !loading && <p className="text-gray-500">{t('no_accounts')}</p>}
        {loading && (
        ...
        {accountsData && !loading && (
          <>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {filtered.length === 0 && (
                  <tr><td colSpan={3} className="p-3 text-center text-gray-500">{t('no_match')}</td></tr>
                )}
