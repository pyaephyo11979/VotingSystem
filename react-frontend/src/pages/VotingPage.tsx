    ...
    const fetchData = async () => {
      ...
      if (eventId && eventPassword) {
        try {
          ...
          const list = (response.candidates || []).map((c:CandidateApi, idx:number) => ({
            id: c.id || c.candidateId || c.name || String(idx),
            name: c.name || c.candidateName || c.id || `Candidate ${idx+1}`,
            photo: c.photo || c.image || null
          }));
          setCandidates(list);
      setEventMeta({ eventId, eventName: localStorage.getItem("eventName") || undefined });
          if (userId) {
            try {
  ...
  if (needsLogin) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 px-4 py-8">
        ...
        <p className="text-gray-600 mb-6 max-w-md text-center">You must log in with your voter credentials before you can view candidates and cast your vote.</p>
        <button onClick={() => navigate('/login')} className="bg-gray-900 text-white px-6 py-3 rounded-lg hover:bg-gray-700 transition font-semibold shadow">Go to Login</button>
      </div>
    );
  }
