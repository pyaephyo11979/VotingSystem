...
const envBase = (import.meta as unknown as { env: ViteEnv }).env
  .VITE_API_BASE_URL;
const base_url =
  envBase || window.__API_BASE_URL__ || "http://localhost:8080/api/events";
...
async function request<T>(input: RequestInfo, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok || !json.success) {
    ...
    if (
      res.status === 404 &&
      json?.error?.code === "EVENT_OR_PASSWORD_INVALID"
    ) {
...
export const createEvent = (eventName: string) =>
  request<{ eventId: string; eventName: string; eventPassword: string }>(
    `${base_url}/create`,
    {
      method: "POST",
...
export const updateCandidate = (
  ...
  if (photo) formData.append("photo", photo);
  return request<{ candidateId: string; eventId: string; updated: boolean }>(
...
// Accounts
export const createAccounts = (eventId: string, eventSize: number) =>
  request<{ eventId: string; accountsCreated: number; accounts: AccountDto[] }>(
...
interface AccountsMetaResponse {
  ...
  accounts?: unknown;
  __accounts_json?: string;
}
export const getAccounts = async (eventId: string) => {
  const meta = await request<AccountsMetaResponse>(
    `${base_url}/${eventId}/accounts`,
  );
...
// Vote status
export const getVoteStatus = (eventId: string, userId: string) =>
  request<{ eventId: string; userId: string; hasVoted: boolean }>(
...
export const getVoterCandidates = (eventId: string, password: string) =>
  getCandidates(eventId, password);
